 ...
 * @param errorFilter A function to filter the error. By default, it filters the error when the query is rejected.
 * @param errorContent The content to display when the query is rejected. By default, it [throws][CatchScope.Throw] the error.
...
inline fun <T> Await(
    ...
    errorFilter: (DataModel<*>) -> Boolean = { it.reply.isNone },
    errorContent: @Composable CatchScope.(err: Throwable) -> Unit = { Throw(error = it) },
    crossinline content: @Composable (data: T) -> Unit
) {
    when (val reply = state.reply) {
        is Reply.Some -> content(reply.value)
        is Reply.None -> Catch(state, filter = errorFilter, content = errorContent)
    }
...
inline fun <T1, T2> Await(
    ...
    when (val reply = Reply.combine(state1.reply, state2.reply, ::Pair)) {
        ...
        is Reply.None -> Catch(state1, state2, filter = errorFilter, content = errorContent)
    }
...
inline fun <T1, T2, T3> Await(
    ...
    when (val reply = Reply.combine(state1.reply, state2.reply, state3.reply, ::Triple)) {
        ...
        is Reply.None -> Catch(state1, state2, state3, filter = errorFilter, content = errorContent)
    }
