...
class CharSequenceRuleTest : UnitTest() {
    ...
    @Test
    fun rule_custom() {
        val custom = CharSequenceRule({ startsWith("test") }) { "Invalid!" }
        val rule = testRule {
            extend(custom)
        }
    ...
    private fun testRule(block: CharSequenceRuleBuilder.() -> Unit): CharSequenceRule = createTestRule(block)
}
